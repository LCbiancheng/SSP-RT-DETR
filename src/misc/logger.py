...
from .console import is_concise_logging
...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        should_print = isinstance(print_freq, int) and print_freq > 0 and not concise_mode
        ...
        for obj in iterable:
            ...
            if should_print and (i % print_freq == 0 or i == len(iterable) - 1):
                eta_seconds = iter_time.global_avg * (len(iterable) - i)
                eta_string = str(datetime.timedelta(seconds=int(eta_seconds)))
                done = i + 1
                fill = int(round(bar_width * done / max(len(iterable), 1)))
                bar = '[' + '#' * fill + '.' * (bar_width - fill) + ']'
                if torch.cuda.is_available():
                    print(log_msg.format(
                        done, len(iterable), bar=bar, eta=eta_string,
                        meters=str(self),
                        memory=torch.cuda.max_memory_allocated() / MB))
                else:
                    print(log_msg.format(
                        done, len(iterable), bar=bar, eta=eta_string, meters=str(self)))
            i += 1
            end = time.time()
