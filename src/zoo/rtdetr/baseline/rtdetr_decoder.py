...
__all__ = ['BaselineRTDETRTransformer']
...
@register
class BaselineRTDETRTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        if self.pg_o2m_num_groups < 1:
            raise ValueError('pg_o2m_num_groups must be >= 1')
        if self.pg_o2m_mask_ratio < 0.0 or self.pg_o2m_mask_ratio > 1.0:
            raise ValueError('pg_o2m_mask_ratio must be in [0, 1]')
        if self.pg_o2m_noise_std < 0:
            raise ValueError('pg_o2m_noise_std must be >= 0')
    ...
    def forward(self, feats, targets=None):
        ...
        if self.training and self.use_pg_o2m:
            ...
            for g in range(N):
                ...
                pg_bboxes, pg_logits = self.decoder(
                    perturbed_target,
                    perturbed_ref,
                    memory,
                    spatial_shapes,
