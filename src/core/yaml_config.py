...
class YAMLConfig(BaseConfig):
    def __init__(self, cfg_path: str, **kwargs) -> None:
        ...
        self.result_csv_name = cfg.get('result_csv_name', 'experiment_results.csv')
        ...
        self.early_stopping = copy.deepcopy(cfg.get('early_stopping', dict()))
        
        from .yaml_utils import GLOBAL_CONFIG
        if 'num_classes' in cfg:
            GLOBAL_CONFIG['num_classes'] = cfg['num_classes']
        elif 'dataset' in cfg and 'num_classes' in cfg['dataset']:
            GLOBAL_CONFIG['num_classes'] = cfg['dataset']['num_classes']
    ...
    def get_optim_params(cfg: dict, model: nn.Module):
        ...
        # Get global learning rate settings
        global_lr = cfg.get('lr', 1e-3)
        global_weight_decay = cfg.get('weight_decay', 0.)
        ...
        for pg in cfg['params']:
            ...
            # Set learning rate for parameter group; prefer group-specific settings, otherwise use global settings
            param_group = pg.copy()
            param_group['params'] = params.values()
            param_group.setdefault('lr', global_lr)
            param_group.setdefault('weight_decay', global_weight_decay)
        ...
        if len(visited) < len(names):
            ...
            # Set global learning rate for unmatched parameters
            param_groups.append({'params': params.values(), 'lr': global_lr, 'weight_decay': global_weight_decay})
            visited.extend(list(params.keys()))
